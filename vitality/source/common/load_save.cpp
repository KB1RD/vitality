...
namespace {
  const std::string kLinuxUserDataDirectory = "~/.local/share/vitality/";
...
File LoadSave::getDataDirectory() {
  ...
  if (!xdg_data_home.trim().isEmpty())
    directory = File(xdg_data_home).getChildFile("Vitality");

#elif defined(__APPLE__)
  File home_directory = File::getSpecialLocation(File::userHomeDirectory);
  File directory = home_directory.getChildFile("Music").getChildFile("Vitality");
#else
  File documents_dir = File::getSpecialLocation(File::userDocumentsDirectory);
  File directory = documents_dir.getChildFile("Vitality");
#endif
